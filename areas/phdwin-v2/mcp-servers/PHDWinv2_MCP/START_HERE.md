...
Bundled reference artifacts include the cleared PHDWin output definitions spreadsheet:

- `reference/phdwin-v2/Phdwinout definitions_complete.xls`

Raw database templates are intentionally not bundled in the Cowork plugin. Keep cleared `.accdb` templates outside the plugin checkout and generate local SQLite fixtures after install.
...
- optionally export an Aries `.accdb` from an external cleared template when Access ODBC is available
- create a local scrubbed PHDWin SQLite review fixture without Access ODBC
...
The plugin avoids bundled raw database templates because highly-compressible `.sqlite`, `.db`, `.mdb`, and `.accdb` files can trip Cowork's plugin compression-ratio guard. For no-driver review workflows, use the `create_phdwin_review_template` MCP tool to generate a local synthetic one-lease PHDWin review fixture with the 76 `PHD_`/`MOD_` source tables. It is fabricated and sanitized; do not treat it as converted client data or as a column-perfect native export.

For optional Aries `.accdb` export, keep a cleared Aries Access template outside the plugin and pass `template_accdb_path` to `export_aries_to_accdb`, or set `ARIES_TEMPLATE_ACCDB_PATH`.
...
To test SQLite interaction without client data, generate a local synthetic fixture:

```text
Use the phdwin-v2 MCP server. Run create_phdwin_review_template for C:\Path\To\phdwin_review_template.sqlite, then inspect that generated SQLite file.
```
